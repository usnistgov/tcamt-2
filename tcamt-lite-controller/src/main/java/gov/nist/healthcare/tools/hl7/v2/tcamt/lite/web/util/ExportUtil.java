...
public class ExportUtil {
	...
	private String genPackagePagesForTestCase(TestPlan tp, TestCase tc, String packageBodyHTML, String index,
		...
		for (int i = 0; i < tc.getTeststeps().size(); i++) {
			...
			packageBodyHTML = genPackagePagesForTestStep(tp, child, packageBodyHTML, index + "." + (i + 1),
					testStoryConfigurationService, tc.getName(), profileService);
		}
	...
	private String genPackagePagesForTestStep(TestPlan tp, TestStep ts, String packageBodyHTML, String index,
			TestStoryConfigurationService testStoryConfigurationService, String testCaseName,
			ProfileService profileService) throws Exception {
		...
		if (ts != null && ts.getEr7Message() != null && ts.getIntegrationProfileId() != null) {
			...
			TestStepSupplementXMLsOutput testStepSupplementXMLsOutput = new GenerationUtil().getSupplementXMLs(params,
					profileService.findOne(params.getIntegrationProfileId()));

			if (testStepSupplementXMLsOutput != null && testStepSupplementXMLsOutput.getMessageContentsXMLStr() != null
					&& !testStepSupplementXMLsOutput.getMessageContentsXMLStr().equals("")) {
				...
				InputStream sourceInputStream = new ByteArrayInputStream(
						testStepSupplementXMLsOutput.getMessageContentsXMLStr().getBytes());
			...
			if (testStepSupplementXMLsOutput != null && testStepSupplementXMLsOutput.getNistXMLStr() != null
					&& !testStepSupplementXMLsOutput.getNistXMLStr().equals("")) {
				if (ts.getTdsXSL() != null && !ts.getTdsXSL().equals("")) {
					...
					InputStream sourceInputStream = new ByteArrayInputStream(
							testStepSupplementXMLsOutput.getNistXMLStr().getBytes());
					Reader xsltReader = new InputStreamReader(xsltInputStream, "UTF-8");
					Reader sourceReader = new InputStreamReader(sourceInputStream, "UTF-8");
					String xsltStr = IOUtils.toString(xsltReader);
					String sourceStr = IOUtils.toString(sourceReader);
				...
				if (ts.getJdXSL() != null && !ts.getJdXSL().equals("")) {
					...
					if (params.getJdXSL().equals("IZ52-IZ33NF-IZ33TM-IZ33PD-JurorDocument")) {
						...
						if (qak2.equals("OK")) {
							String jdTemplate = IOUtils.toString(
									classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_OK.txt"));
							...
							String immunizationScheduleUsed = this.findDataByPathAndCondition(nistXMLDom, "OBX",
									"OBX.3.1", "59779-9", "OBX.5.2", 0);
							...
							NodeList COMPLETE_HISTORYGroups = nistXMLDom
									.getElementsByTagName("RSP_K11.COMPLETE_HISTORY");
							if (COMPLETE_HISTORYGroups != null && COMPLETE_HISTORYGroups.getLength() > 0) {
								for (int i = 0; i < COMPLETE_HISTORYGroups.getLength(); i++) {
									...
									if (value_RXA_5_1.equals("998")) {
											...
											for (int j = obxSegments.getLength() - 1; j > -1; j--) {
												...
												if (this.findDataByPath(obxSegment, "OBX.3.1").equals("30956-7")) {
													...
													if (immunizationForecastDueDate.equals("NOT_Found")
														...
														trHTML = trHTML + "<td style=\"text-align: center;\">"
																+ this.changeDateFormat(immunizationForecastDueDate)
																+ "</td>";
													}
												...
												} else if (this.findDataByPath(obxSegment, "OBX.3.1")
														.equals("30980-7")) {
													immunizationForecastDueDate = this.findDataByPath(obxSegment,
															"OBX.5.1");
												} else if (this.findDataByPath(obxSegment, "OBX.3.1")
														.equals("30981-5")) {
													immunizationForecastEarliestDatetoGive = this
															.findDataByPath(obxSegment, "OBX.5.1");
												} else if (this.findDataByPath(obxSegment, "OBX.3.1")
														.equals("59777-3")) {
													immunizationForecastLatestDatetoGive = this
															.findDataByPath(obxSegment, "OBX.5.1");
												}
											}
										}
										...
										String immunizationHistoryDateAdministered = this
												.findDataByPath(COMPLETE_HISTORYGroup, "RXA.3");
										String immunizationHistoryValidDose = "";
										String immunizationHistoryValidityReason = "";
										String immunizationHistoryCompletionStatus = this
												.findDataByPath(COMPLETE_HISTORYGroup, "RXA.20");
											...
											for (int j = obxSegments.getLength() - 1; j > -1; j--) {
												...
												if (this.findDataByPath(obxSegment, "OBX.3.1").equals("30956-7")) {
													...
													immunizationHistory = immunizationHistory + trHTML;
														...
														.equals("59781-5")) {
													immunizationHistoryValidDose = this.findDataByPath(obxSegment,
															"OBX.5.1");
												} else if (this.findDataByPath(obxSegment, "OBX.3.1")
														.equals("30982-3")) {
							...
							String jdTemplate = IOUtils.toString(
									classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_NF.txt"));
							jdTemplate = jdTemplate.replace("$testcasename$", testCaseName);
							...
							String jdTemplate = IOUtils.toString(
									classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_PD.txt"));
							jdTemplate = jdTemplate.replace("$testcasename$", testCaseName);
							String patientIdentifier = this.findDataByPath(nistXMLDom, "QPD.3.1");
							jdTemplate = jdTemplate.replace("$patientIdentifier$", patientIdentifier);
							...
							String jdTemplate = IOUtils.toString(
									classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_TM.txt"));
							jdTemplate = jdTemplate.replace("$testcasename$", testCaseName);
							String patientIdentifier = this.findDataByPath(nistXMLDom, "QPD.3.1");
							jdTemplate = jdTemplate.replace("$patientIdentifier$", patientIdentifier);
	...
	private void genProfileAsXML(ZipOutputStream out, TestPlan tp, String profileId, String profileXML)
			throws Exception {
		byte[] buf = new byte[1024];
		out.putNextEntry(
				new ZipEntry("Global" + File.separator + "Profiles" + File.separator + profileId + "_Profile.xml"));
	...
	private void genValueSetAsXML(ZipOutputStream out, TestPlan tp, String id, String valueSetXMLFileStr)
			throws IOException {
		byte[] buf = new byte[1024];
	...
	private void genConstraintsAsXML(ZipOutputStream out, TestPlan tp, String id, String constraintsXMLFileStr)
			throws IOException {
		byte[] buf = new byte[1024];
		out.putNextEntry(
	...
	private void generateTestPlanRBTestCase(ZipOutputStream out, TestCase tc, String path, TestPlan tp,
		...
		for (int i = 0; i < tc.getTeststeps().size(); i++) {
			TestStep child = tc.getTeststeps().get(i);
			generateTestPlanRBTestStep(out, child, tcPath, tp, testStoryConfigurationService, i + 1, profileService,
					tc.getName());
		}
	}
		...
		if (ts.getConformanceProfileId() != null && !ts.getConformanceProfileId().equals("")) {
			...
			if (testStepSupplementXMLsOutput != null) {
				if (testStepSupplementXMLsOutput.getNistXMLStr() != null) {
					if (ts.getTdsXSL() != null && !ts.getTdsXSL().equals("")) {
						this.generateTestDataSpecification(out, testStepSupplementXMLsOutput.getNistXMLStr(),
								ts.getTdsXSL(), stepPath, "ng-tab-html");
						this.generateTestDataSpecification(out, testStepSupplementXMLsOutput.getNistXMLStr(),
								ts.getTdsXSL(), stepPath, "plain");
					}
					if (ts.getJdXSL() != null && !ts.getJdXSL().equals("")) {
						this.generateJurorDocument(out, ts.getJdXSL(), testStepSupplementXMLsOutput.getNistXMLStr(),
								stepPath, "ng-tab-html");
						this.generateJurorDocument(out, ts.getJdXSL(), testStepSupplementXMLsOutput.getNistXMLStr(),
								stepPath, "plain");
					}
				}
				if (testStepSupplementXMLsOutput.getMessageContentsXMLStr() != null) {
					this.generateMessageContent(out, testStepSupplementXMLsOutput.getMessageContentsXMLStr(), stepPath,
							"ng-tab-html");
					this.generateMessageContent(out, testStepSupplementXMLsOutput.getMessageContentsXMLStr(), stepPath,
							"plain");
				}
	...
	private void generateTestPlanRB(ZipOutputStream out, TestPlan tp,
		...
		for (int i = 0; i < tp.getChildren().size(); i++) {
			...
			if (child instanceof TestCaseGroup) {
				generateTestPlanRBTestGroup(out, (TestCaseGroup) child, rootPath, tp, testStoryConfigurationService,
						i + 1, profileService);
			} else if (child instanceof TestCase) {
				generateTestPlanRBTestCase(out, (TestCase) child, rootPath, tp, testStoryConfigurationService, i + 1,
						profileService);
			}
		}
	}

	private void generateJurorDocument(ZipOutputStream out, String jdXSL, String nistXMLStr, String teststepPath,
			String option) throws IOException {
		ClassLoader classLoader = getClass().getClassLoader();
		InputStream is = classLoader.getResourceAsStream("xsl" + File.separator + jdXSL + ".xsl");
		String mcXSL = null;
		if (is != null) {
			byte[] buf = new byte[1024];
	...
	private void generateTestDataSpecification(ZipOutputStream out, String nistXML, String tdXSL, String teststepPath,
			String option) throws IOException {
		ClassLoader classLoader = getClass().getClassLoader();
		InputStream is = classLoader.getResourceAsStream("xsl" + File.separator + tdXSL + ".xsl");
		String mcXSL = null;
		if (is != null) {
			byte[] buf = new byte[1024];
		...
		if (path == null) {
			if (option.equals("ng-tab-html")) {
				out.putNextEntry(new ZipEntry(
						"Contextbased" + File.separator + tp.getName() + File.separator + "TestStory.html"));
			} else {
				out.putNextEntry(new ZipEntry(
						"Contextbased" + File.separator + tp.getName() + File.separator + "TestStoryPDF.html"));
			}
		} else {
			if (option.equals("ng-tab-html")) {
	...
	public InputStream exportProfileXMLZip(Set<String> keySet, ProfileService profileService, Long rand)
			throws IOException {
	...
	public InputStream[] exportProfileXMLArrayZip(String id, ProfileService profileService) throws IOException {
