		...
		<tr>
			...
			<xsl:choose>
				<xsl:when test="exists(QPD.4)">
					<td>
						<xsl:value-of select="concat(QPD.4/QPD.4.2, ' ', QPD.4/QPD.4.3, ' ', QPD.4/QPD.4.1/QPD.4.1.1)"/>
					</td>
				</xsl:when>
				<xsl:otherwise>
					<td bgcolor="#D2D2D2"/>
							...
							<table>
								...
								<tbody>
									...
									<xsl:for-each select="//PID">
										<tr>
											<xsl:call-template name="testExistence">
												<xsl:with-param name="node" select="PID.3[1]/PID.3.1"/>
											</xsl:call-template>
											<xsl:choose>
												<xsl:when test="exists(PID.5)">
													<td>
														<xsl:value-of select="concat(PID.5/PID.5.2, ' ', PID.5/PID.5.3, ' ', PID.5/PID.5.1/PID.5.1.1)"/>
													</td>
												</xsl:when>
												<xsl:otherwise>
													<td bgcolor="#D2D2D2"/>
											...
											<xsl:call-template name="testExistence">
												<xsl:with-param name="node" select="util:format-date(PID.7/PID.7.1)"/>
											</xsl:call-template>
											<xsl:call-template name="testExistence">
												<xsl:with-param name="node" select="util:gender(PID.8)"/>
											</xsl:call-template>
											<xsl:call-template name="commentTemplate"/>
										</tr>
									</xsl:for-each>
							...
							<xsl:if test="//OBX.3.1[. = '59779-9']">
								<table>
									<tbody>
										...
										<tr>
											<xsl:call-template name="testExistence">
												<xsl:with-param name="node" select="distinct-values(//OBX.3.1[. = '59779-9']/../../OBX.5/OBX.5.2)"/>
											</xsl:call-template>
											<xsl:call-template name="commentTemplate"/>
										</tr>
									</tbody>
										...
										<xsl:for-each select="//RXA.5.1[. != '998']/../../..">
											...
											<xsl:for-each select="*[self::RSP_K11.HISTORY_FORECAST_OBSERVATION or self::RSP_K11.OBSERVATION]/OBX/OBX.3/OBX.3.1[. = '30956-7']/../../..">
												...
												<tr>
													...
													<xsl:call-template name="testExistence">
														<xsl:with-param name="node" select="../RXA/RXA.5/RXA.5.2"/>
													</xsl:call-template>
													<xsl:call-template name="testExistence">
													...
													<xsl:choose>
														...
														<xsl:when test="following-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION /OBX/OBX.3/OBX.3.1[. = '59781-5']/../../..[count(preceding-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION /OBX/OBX.3/OBX.3.1[. = '30956-7']/../../..) = $position]">
															<xsl:for-each select="following-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION/OBX/OBX.3/OBX.3.1[. = '59781-5']/../../..[count(preceding-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION/OBX/OBX.3/OBX.3.1[. = '30956-7']/../../..) = $position]">
																<xsl:if test="position() = 1">
																	<xsl:call-template name="testExistence">
																		<xsl:with-param name="node" select="util:validDose(OBX/OBX.5/OBX.5.1)"/>
																	</xsl:call-template>
													...
													<xsl:choose>
														...
														<xsl:when test="following-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION/OBX/OBX.3/OBX.3.1[. = '30982-3']/../../..[count(preceding-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION/OBX/OBX.3/OBX.3.1[. = '30956-7']/../../..) = $position]">
															<xsl:for-each select="following-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION/OBX/OBX.3/OBX.3.1[. = '30982-3']/../../..[count(preceding-sibling::RSP_K11.HISTORY_FORECAST_OBSERVATION/OBX/OBX.3/OBX.3.1[. = '30956-7']/../../..) = $position]">
																<xsl:if test="position() = 1">
																	<xsl:copy-of select="util:formatData(OBX/OBX.5/OBX.5.1)"/>
																</xsl:if>
															</xsl:for-each>
													...
													<xsl:call-template name="testExistence">
														<xsl:with-param name="node" select="util:completionStatus(../RXA/RXA.20)"/>
													</xsl:call-template>
													<xsl:call-template name="commentTemplate"/>
												</tr>
											</xsl:for-each>
