...
.11 SS update for Observation Value condition
	...
	<xsl:template match="PID" mode="Syndromic">
		...
		<xsl:value-of select="util:elements($ind1)"/>
			<xsl:choose>
				<xsl:when test=".//PID.5[2]/PID.5.7 ='U' or .//PID.5[2]/PID.5.7 ='S'">
					<xsl:value-of select="util:element('Name', util:valueset(.//PID.5[2]/PID.5.7, 'HL70200'), $ind1)"/>
				</xsl:when>
				<xsl:when test=".//PID.5[1]/PID.5.7 ='U' or .//PID.5[1]/PID.5.7 ='S'">
					<xsl:value-of select="util:element('Name', util:valueset(.//PID.5[1]/PID.5.7, 'HL70200'), $ind1)"/>
				</xsl:when>
				<xsl:otherwise>
					<!-- TODO -->
				</xsl:otherwise>
		...
		<!-- Insurance -->
		<xsl:for-each select="..//IN1">
			<xsl:variable name="insurance">
				<xsl:choose>
					<xsl:when test=".//IN1.2.2  != ''">
						<xsl:value-of select=".//IN1.2.2"/>
					</xsl:when>
					<xsl:when test=".//IN1.2.1  != ''">
						<xsl:value-of select=".//IN1.2.1"/>
					</xsl:when>
				</xsl:choose>
			</xsl:variable>		
		<xsl:value-of select="util:element('Insurance Plan ID', $insurance, $ind1)"/>
	...
	<xsl:template match="PV1" mode="Syndromic">
		...
		<xsl:for-each select="..//PR1">
			<xsl:variable name="procedure">
				<xsl:choose>
					<xsl:when test=".//PR1.3.2  != ''">
						<xsl:value-of select=".//PR1.3.2"/>
					</xsl:when>
					<xsl:when test=".//PR1.3.1  != ''">
						<xsl:value-of select=".//PR1.3.1"/>
					</xsl:when>
				</xsl:choose>
			</xsl:variable>		
		<xsl:value-of select="util:element('Procedure', $procedure, $ind1)"/>
		<xsl:value-of select="util:element('Procedure Date/Time', util:format-time(.//PR1.5.1), $ind1)"/>
		</xsl:for-each>
		<xsl:value-of select="util:end-elements($ind1, $vertical-orientation, false())"/>
	</xsl:template>
	...
	<xsl:template match="OBX" mode="Syndromic">
		...
		<xsl:value-of select="util:elements($ind1)"/>
		<xsl:value-of select="util:element('Observation  Date/Time', util:format-time(.//OBX.14.1), $ind1)"/>
		...
		<xsl:variable name="observation-value">
			<xsl:choose>			
				...
				<xsl:when test=".//OBX.3.1 = '11283-9'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.5.2, .//OBX.5.1, 'PHVS_EmergencySeverityIndexAcuity_CDC')"/>
				</xsl:when>
				<xsl:when test=".//OBX.3.1 = '11449-6'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.5.2, .//OBX.5.1, 'HL70532_SS')"/>
				</xsl:when>


				<!-- XAD -->
				<xsl:when test=".//OBX.3.1 = 'SS002'">
					<xsl:value-of select="util:format-address-2(.//OBX.5.1, .//OBX.5.3, util:valueset(.//OBX.5.4, 'PHVS_State_FIPS_5-2'), .//OBX.5.5, util:valueset(.//OBX.5.6, 'PHVS_Country_ISO_3166-1'), .//OBX.5.9)"/>
				</xsl:when>
				
				<!-- TS -->
				<xsl:when test=".//OBX.3.1 = '11368-8'">
					<!-- TODO -->
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="units">
			<xsl:choose>
				...
				<xsl:when test=".//OBX.3.1 = '11289-6'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.6.2 , .//OBX.6.1, 'PHVS_TemperatureUnit_UCUM')"/>
				</xsl:when>
				<xsl:when test=".//OBX.3.1 = '59408-5'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.6.2 , .//OBX.6.1, 'PHVS_PulseOximetryUnit_UCUM')"/>
				</xsl:when>
				<xsl:when test=".//OBX.3.1 = '8480-6'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.6.2 , .//OBX.6.1, 'PHVS_BloodPressureUnit_UCUM')"/>
				</xsl:when>
				<xsl:when test=".//OBX.3.1 = '8462-4'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.6.2 , .//OBX.6.1, 'PHVS_BloodPressureUnit_UCUM')"/>
				</xsl:when>
				<xsl:when test=".//OBX.3.1 = '39156-5'">
					<xsl:value-of select="util:value-or-valueset(.//OBX.6.2 , .//OBX.6.1, 'BMI')"/>
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:format-address-2">
		...
		<xsl:value-of select="concat(util:format-with-space($street), util:format-with-space($city), util:format-with-space($state), util:format-with-space($zip), util:format-with-space($country), util:format-with-space($county))"/>
	</xsl:function>
	...
	<!-- returns the value ($val) if non-empty. Otherwise returns the value mapped to the key ($key) in the value set $tablename -->
	...
	<xsl:variable xmlns:fo="http://www.w3.org/1999/XSL/Format" name="HL7Tables">
        <Tables xmlns="">
            ...
            <TableDefinition Codesys="PHVS_EmergencySeverityIndexAcuity_CDC" Id="PHVS_EmergencySeverityIndexAcuity_CDC" Name="Emergency Severity Index Acuity">
                <t c="1" d="Resuscitation"/>
                <t c="2" d="Emergent"/>
                <t c="3" d="Urgent"/>
                <t c="4" d="Less Urgent"/>
                <t c="5" d="Nonurgent"/>
            </TableDefinition>
              <TableDefinition Codesys="HL70532_SS" Id="HL70532_SS" Name="Expanded yes no indicator">
                <t c="ASKU" d="asked but unknown"/>
                <t c="N" d="No"/>
                <t c="NA" d="not applicable"/>
                <t c="NASK" d="not asked"/>
                <t c="NAV" d="temporarily unavailable"/>
                <t c="NI" d="No Information"/>
                <t c="NP" d="not present"/>
                <t c="UNK" d="unknown"/>
                <t c="Y" d="Yes"/>
            </TableDefinition>
        
              
            <TableDefinition Codesys="PHVS_BloodPressureUnit_UCUM" Id="PHVS_BloodPressureUnit_UCUM" Name="Blood Pressure Unit">
                <t c="mm[Hg]" d="MilliMeters of Mercury [Blood Pressure Unit]"/>
            </TableDefinition>
            <TableDefinition Codesys="BMI" Id="BMI" Name="BMI Unit">
                <t c="kg/m2" d="kilogram / (meter squared)"/>
            </TableDefinition>    
        <!-- =================================================================================================================== -->
...
<!-- =================================================================================================================== -->
            ...
            <TableDefinition Codesys="PHVS_ObservationIdentifier_SyndromicSurveillance" Id="Observation Identifier Syndromic Surveillance" Name="Observation Identifier (Syndromic Surveillance)" Oid="2.16.840.1.114222.4.11.3589" Type="Local" Version="2">
                ...
                <t c="10160-0" d="Medication use Hx Reported"/>
				<t c="10182-4" d="Travel Hx Reported"/>
				<t c="11283-9" d="Acuity assessment [Function] at First encounter"/>
				<t c="11449-6" d="Pregnancy status - reported"/>
				<t c="11450-4" d="Problem list Reported"/>
				<t c="39156-5" d="BMI"/>
				<t c="54582-2" d="Provider type [MDSv3]"/>
				<t c="8677-7" d="Medication use Hx Reported"/>
				
				<!-- PHVS_VitalSignResult_HITSP -->
				<t c="2710-2" d="Oxygen saturation:SFr:Pt:BldC:Qn:Oximetry"/>
				<t c="8287-5" d="Circumference.occipital-frontal:Len:Pt:Head:Qn:Tape measure"/>
				<t c="8306-3" d="Body height-lying:Len:Pt:Patient:Qn:"/>
				<t c="8310-5" d="Body temperature:Temp:Pt:Patient:Qn:"/>
				<t c="8462-4" d="Intravascular diastolic:Pres:Pt:Arterial system:Qn:"/>
				<t c="8480-6" d="Intravascular systolic:Pres:Pt:Arterial system:Qn:"/>
				<t c="8867-4" d="Heart beat:NRat:Pt:XXX:Qn:"/>
				<t c="9279-1" d="Breaths:NRat:Pt:Respiratory system:Qn:"/>
            </TableDefinition>
